...
def testCell(x, y, sensor, sensors):
    """
...
def checkBorder(data):
    ...
    for i in range(d + 1):
        testCell(xs + i, ys - d - 1 + i, sensor, sensors)
        testCell(xs + d + 1 - i, ys - i, sensor, sensors)
        testCell(xs - i, ys + d + 1 - i, sensor, sensors)
        testCell(xs - d - 1 + i, ys + i, sensor, sensors)
...
if __name__ == "__main__":
    ...
    p = Pool(15)
    data = [(s,sensors) for s in sensors]
    p.map(checkBorder, data)
